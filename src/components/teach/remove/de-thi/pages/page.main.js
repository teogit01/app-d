...
function PageDeThi(props) {
    ...
    const changeSearch = (e) => {
        ...
        typingTimeoutRef.current = setTimeout(() => {
            ...
            let result = []
            const newDethis = dethis.filter(dethi => dethi.ma.toUpperCase().indexOf(value.toUpperCase()) !== -1)
            if (newDethis.length === 0) {
                const dethi_mon = dethis.filter(dethi => dethi.mon.ten.toUpperCase().indexOf(value.toUpperCase()) !== -1)
                result = dethi_mon
                setDethiSelected(dethi_mon.length > 0 ? dethi_mon[0] : '')
            } else {
                result = newDethis
                setDethiSelected(newDethis.length > 0 ? newDethis[0] : '')
            }
            setDethiRender(result)
            //}
        }, 1000)
    }
    const [tooltipOpen, setTooltipOpen] = useState(false);
    const toggleT = () => setTooltipOpen(!tooltipOpen);
    return (
        <div className='page-de-thi'>
            <div className='content-page'>
                <div className='left'>
                    ...
                    <FontAwesomeIcon id="TooltipExample" className='ic-add ic-init add-de' icon="plus" onClick={toggle} />
                    <Tooltip placement="left" isOpen={tooltipOpen} target="TooltipExample" toggle={toggleT}>
                        Tạo đề thi
                    </Tooltip>
                    <FontAwesomeIcon className='ic-add ic-init add-cauhoi' icon="plus" onClick={toggleCauhoi} />
                    {
